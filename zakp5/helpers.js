...
function drawUI() {
	...
	//Combo counter!
	comboCount = comboPartsCount / secondsSpent;
	textAlign(LEFT);
	if (prevComboCount < comboCount - 0.5 && comboCount > 5 || prevComboCount > comboCount - 0.5 && comboCount > 5) {
		drawComboCount();
		comboCounting = true;
	}

	if (comboCount < 5) {
		drawComboCountResets();
		comboCounting = false;
	}
...
function changeImageLogic() {
	...
	if (randnum < 0.166 && finishedChangeImg === false) {
		if (imgToClick === mainClickImages.img1) {
			...
			comboPartsCount = comboPartsCount + 1;
		}
